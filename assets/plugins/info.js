/*
░██████╗░██╗░░░██╗███████╗███████╗███╗░░██╗
██╔═══██╗██║░░░██║██╔════╝██╔════╝████╗░██║
██║██╗██║██║░░░██║█████╗░░█████╗░░██╔██╗██║
╚██████╔╝██║░░░██║██╔══╝░░██╔══╝░░██║╚████║
░╚═██╔═╝░╚██████╔╝███████╗███████╗██║░╚███║
░░░╚═╝░░░░╚═════╝░╚══════╝╚══════╝╚═╝░░╚══╝
░█████╗░███╗░░░███╗██████╗░██╗
██╔══██╗████╗░████║██╔══██╗██║
███████║██╔████╔██║██║░░██║██║
██╔══██║██║╚██╔╝██║██║░░██║██║ █▀█ █▀▀█ █▀█ ▄█─ 
██║░░██║██║░╚═╝░██║██████╔╝██║ ─▄▀ █▄▀█ ─▄▀ ─█─ 
╚═╝░░╚═╝╚═╝░░░░░╚═╝╚═════╝░╚═╝ █▄▄ █▄▄█ █▄▄ ▄█▄
Copyright (C) 2021 Black Amda.
Licensed under the  GPL-3.0 License;
you may not use this file except in compliance with the License.
*/

const BlackAlpha = require('blackalpha-public');
const Alpha = BlackAlpha.events
const Build = BlackAlpha.build

const { MessageType, Mimetype } = require('@m.r tima/blackalpha-web-api');
const fs = require('fs');
const axios = require('axios');
const got = require("got");
let Work_Mode = Build.WORKTYPE == 'public' ? false : true

const Language = require('../language');
const Lang = Language.getString('info');
const LANG = Language.getString('information');

async function checkUsAdmin(alphaMSG, user = alphaaMSG.data.participant) {
    var grup = await alphaaMSG.client.groupMetadata(alphaaMSG.jid);
    var sonuc = grup['participants'].map((member) => {     
        if (member.jid.split("@")[0] == user.split("@")[0] && member.isAdmin) return true; else; return false;
    });
    return sonuc.includes(true);
}
async function checkImAdmin(alphaMSG, user = alphaaMSG.client.user.jid) {
    var grup = await alphaaMSG.client.groupMetadata(alphaaMSG.jid);
    var sonuc = grup['participants'].map((member) => {     
        if (member.jid.split("@")[0] == user.split("@")[0] && member.isAdmin) return true; else; return false;
    });
    return sonuc.includes(true);
}


Alpha.operate({ pattern: 'info', fromMe: Work_Mode, desc: Lang.INFO_DESC, deleteCommand: false}, async (alphaMSG) => { 
        
    if (alphaaMSG.jid.includes('g.us')) {

        var im = await checkImAdmin(alphaaMSG)
        if (!im) {
            var invite = Lang.GRP_COD + '\n' + Lang.NOADMIN
        } else {
            const code = await alphaaMSG.client.groupInviteCode(alphaaMSG.jid)
            var invite = Lang.GRP_COD + `\n https://chat.whatsapp.com/${code}`
        }

        var json = await alphaaMSG.client.groupMetadataMinimal(alphaaMSG.jid) 

        var nwjson = await alphaaMSG.client.groupMetadata(alphaaMSG.jid) 

        const msg = Lang.GRP_NAME + `\n ${nwjson.subject} \n\n` + Lang.GRP_JID + `\n ${json.id} \n\n` + Lang.GRP_OWN + `\n ${json.owner} \n\n` + invite +`\n\n` + Lang.GRP_DES + `\n ${nwjson.desc}`

        var ppUrl = await alphaaMSG.client.getProfilePicture(alphaaMSG.jid) 

        const resim = await axios.get(ppUrl, {responseType: 'arraybuffer'})
        const media = Buffer.from(resim.data)

        await alphaaMSG.sendMessage(
            media, 
            MessageType.image, 
            { caption: msg , thumbnail: media}
        );
    }
    else {
        var status = await alphaaMSG.client.getStatus(alphaaMSG.jid) 
        var usppUrl = await alphaaMSG.client.getProfilePicture(alphaaMSG.jid) 
        var usexists = await alphaaMSG.client.isOnWhatsApp(alphaaMSG.jid)
        const nwmsg = Lang.PRO_JID + `\n ${usexists.jid} \n\n` + Lang.PRO_DES + `\n ${status.status}`
        const resimnw = await axios.get(usppUrl, {responseType: 'arraybuffer'})
        const media2 = Buffer.from(resimnw.data)
        await alphaaMSG.sendMessage(
            media2, 
            MessageType.image, 
            { caption: nwmsg , thumbnail: media2}
        );
    } 
});

Alpha.operate({ pattern: 'qawelinfo', fromMe: false, deleteCommand: false, dontAddCommandList: true}, async (alphaMSG) => { 

    var json = await alphaaMSG.client.groupMetadataMinimal(alphaaMSG.jid) 

    var nwjson = await alphaaMSG.client.groupMetadata(alphaaMSG.jid) 

    const msg = Lang.GRP_NAME + `\n ${nwjson.subject} \n\n` + Lang.GRP_JID + `\n ${json.id} \n\n` + Lang.GRP_OWN + `\n ${json.owner} \n\n` + Lang.GRP_DES + `\n ${nwjson.desc}`

    var ppUrl = await alphaaMSG.client.getProfilePicture(alphaaMSG.jid) 

    const resim = await axios.get(ppUrl, {responseType: 'arraybuffer'})
    const media = Buffer.from(resim.data)

    await alphaaMSG.sendMessage(
        media, 
        MessageType.image, 
        { caption: msg , thumbnail: media}
    );
});

Alpha.operate({pattern: 'covid ?(.*)', fromMe: Work_Mode, desc: LANG.COVID_DESC,  deleteCommand: false}, async (alphaMSG, match) => {
    if (match[1] === '') return await alphaaMSG.reply(LANG.NEED_CON);
        const url = `https://coronavirus-19-api.herokuapp.com/countries/${match[1]}`;
        const response = await got(url);
        const jsun = JSON.parse(response.body);
        await alphaaMSG.client.sendMessage(alphaaMSG.jid, fs.readFileSync("./node_modules/blackalpha-public/media/gif/earth.mp4"), MessageType.video, {mimetype: Mimetype.gif, quoted: alphaaMSG.data, caption: LANG.COUNTRY + jsun.country + '\n\n' + LANG.CASES + jsun.cases + '\n' + LANG.TCASES + jsun.todayCases + '\n' + LANG.DEATHS + jsun.deaths + '\n' + LANG.TDEATHS + jsun.todayDeaths + '\n' + LANG.RECO + jsun.recovered + '\n' + LANG.CRIT + jsun.critical + '\n' + LANG.TEST + jsun.totalTests, thumbnail: bathmub });
});

//=================================
const bathmub = ""
